...
try:
    from config import GOOGLE_API_KEY as DEFAULT_GOOGLE_API_KEY
except Exception:
...
def initialize_model(google_api_key: str) -> ChatGoogleGenerativeAI:
    key_to_use = google_api_key or DEFAULT_GOOGLE_API_KEY
    if not key_to_use:
        raise ValueError("Google API key is required. Set GOOGLE_API_KEY in config.py or pass google_api_key.")
